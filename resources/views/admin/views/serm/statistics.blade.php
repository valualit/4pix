...
@section('content')
	<section class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				{{Breadcrumbs::render('admin.serm.statistics',$project,$setdate)}}
			</div>
		</div><!-- /.container-fluid -->
    </section>
					...
					<div class="card-header pl-0">
						...
						<div class="d-inline form-inline">
							<select class="d-inline form-control form-control-sm" style="min-width:120px" id="DateList">
								...
								<option value="{{route('admin.serm.statistics', $project->id)}}?date={{$date}}" {{$date==$setdate?'selected':null}}>{{$date}}</option>
								@endforeach
							</select>
						</div>
